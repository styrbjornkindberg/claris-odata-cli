...
export interface FieldOptions extends CommandOptions {
  action: 'add' | 'delete';
  tableName: string;
  serverId: string;
  database: string;
  ...
  /** Field name for delete action */
  fieldName?: string;
  /** Required for delete action */
  confirm?: boolean;
}

export class FieldCommand extends BaseCommand<FieldOptions> {
  async execute(): Promise<CommandResult> {
    if (this.options.action === 'delete' && !this.options.confirm) {
      return {
        success: false,
        error: 'Pass --confirm to permanently delete this field and its data',
      };
    }
    ...
    try {
      ...
      if (this.options.action === 'add') {
        const data = await client.addFields(this.options.tableName, this.options.fields ?? []);
        return { success: true, data };
      }

      // delete
      await client.deleteField(this.options.tableName, this.options.fieldName!);
      return {
        success: true,

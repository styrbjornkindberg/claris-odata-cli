...
import type { CommandResult, FieldDefinition } from '../types';

export interface TableOptions extends CommandOptions {
...
export class TableCommand extends BaseCommand<TableOptions> {
  async execute(): Promise<CommandResult> {
    if (this.options.action === 'delete' && !this.options.confirm) {
      return {
        success: false,
        error: 'Pass --confirm to delete the table and all its records',
      };
    }
    ...
    try {
      ...
      if (this.options.action === 'create') {
        const data = await client.createTable(this.options.tableName, this.options.fields ?? []);
        return { success: true, data };
      }

      // delete
      await client.deleteTable(this.options.tableName);
      return { success: true, data: { table: this.options.tableName, deleted: true } };
    } catch (error: unknown) {
